...
namespace InStockWebAppPL.Controllers
{
    public class ChangePasswordController : Controller
    {
        ...
        public ChangePasswordController(IEmailSender emailSender, IWebHostEnvironment webHostEnvironmen, UserManager<User> userManager, SignInManager<User> signInManager)
        {
        ...
        public async Task<IActionResult> Index()
        {
            if (signInManager.IsSignedIn(User))
            {
                ...
                body = body.Replace("[Header]", "Welcom In Instock Shopping")
                    .Replace("[Body]", $"Welcome to InStock!We have received a request to change your password \n {randomNumberInRange}")
                    .Replace("[URL]", "/ChangePassword")
                    .Replace("[AncorTitle]", "Go");
                //Use HangFire
                BackgroundJob.Enqueue(() => emailSender.SendEmailAsync(user.Email, "Welcome Login", body));
        ...
        public async Task<IActionResult> ChangeOldPassword(PasswordVM passwordVM)
        {
            if (signInManager.IsSignedIn(User))
            {
                ...
                var result = await userManager.ChangePasswordAsync(user, passwordVM.CurrentPassword, passwordVM.NewPassword);
